...
/** 纯文字描述一餐 → 估算蛋白质、碳水、摄入热量（kcal）与「应计入」的水分（毫升） */
...
- 须依次涵盖（可写成连贯段落，用小标题感弱衔接即可）：
  ...
  (2) 营养结构：蛋白质、碳水、脂肪与摄入热量的大致倾向（定性即可，勿大段复读数值字段）；
...
function normalizeFoodTextIntakeFromResult(raw: unknown): FoodTextIntakeJson {
  ...
  return {
    ...
    calories_kcal: toNonNegativeFiniteNumber(o.calories_kcal ?? o.calories),
    ai_evaluation,
  };
}

/**
 * 根据用户中文饮食描述估算 hydration_ml、protein_g、carbohydrate_g、calories_kcal。
...
export async function parseFoodIntakeFromText(
  ...
  const systemContent = `只返回严格JSON，不要任何解释和markdown（含代码块）。顶层为一个对象，且必须包含 result；result 内字段：food_summary、hydration_ml、protein_g、carbohydrate_g、calories_kcal、ai_evaluation。
...
- protein_g、carbohydrate_g、calories_kcal 为非负数值，无法从描述推断时填 0；calories_kcal 为本餐可食部分估算摄入热量（千卡 kcal），须综合蛋白质、碳水、脂肪与烹饪用油糖等合理估算。
...
function clampDailyIntakeTargetsEstimate(raw: DailyIntakeTargetsEstimateJson): DailyIntakeTargetsEstimateJson {
  ...
  return {
    ...
    calories_kcal: clamp(raw.calories_kcal, 1000, 5000),
...
function normalizeDailyIntakeTargetsEstimateJson(parsed: unknown): DailyIntakeTargetsEstimateJson | null {
  ...
  const calories_kcal = pickNum(o.calories_kcal ?? o.calories);
  if (![hydration_ml, protein_g, carbohydrate_g, calories_kcal].every(Number.isFinite)) return null;
...
/**
 * 根据用户档案与近七日摄入摘要，输出当日水分/蛋白质/碳水/热量目标（JSON）。
...
- calories_kcal：数字，全日摄入热量目标（千卡 kcal），须结合性别、年龄、身高体重、活动量与减脂/增肌目标；一般成年人在 1200～3500 之间，极端情况不宜低于 1000 或高于 5000
- rationale_zh：字符串，1～2 句简体中文，概括你如何综合「用户身体档案」「近7日实际摄入」「本地公式参考值」得到上述目标；勿编造未在上下文中出现的疾病诊断或实验室数据；若信息不足，说明保守处理。

须遵守：
- 结合用户性别、年龄、身高体重、运动与目标（减脂/增肌）与近7日各营养素摄入趋势做微调；若近几日某营养素或热量持续明显偏低或偏高，目标应温和纠正而非极端跳变。
- 若上下文标明今日为「健身日」，蛋白质与碳水可适度上调，水分略增以支持训练；若标明「休息日」，在保障基础营养前提下温和低于健身日（尤其碳水），勿照搬训练日定量。
- 热量目标须结合 BMR/TDEE 常识与用户目标：减脂可适度赤字但勿极端；增肌可适度盈余；若近7日实际摄入持续明显偏离公式参考，目标应温和纠正。
- 输出数值须为合理整数或最多一位小数的数字（建议四舍五入为整数）。`;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const dr = await dispatchZhipuTextChat({
      apiKey: key,
      systemContent,
      userContent: `请根据以下上下文生成 hydration_ml、protein_g、carbohydrate_g、calories_kcal 与 rationale_zh：\n\n${context}`,
      temperature: 0.15,
      maxTokens: 768,
      maxAttempts: 1,
      retryDelayMs: 0,
...
export function normalizeFoodNutritionPayload(raw: unknown): { data: FoodNutritionJson; repaired: boolean } {
  ...
  let calories_kcal = toNonNegativeFiniteNumber(o.calories_kcal ?? o.calories);
...
${FOOD_INTAKE_AI_EVALUATION_GUIDE}
- protein_g、carbohydrate_g、calories_kcal：数字类型，非负；calories_kcal 为本餐/可见主体可食部分估算摄入热量（千卡 kcal）；is_food 为 0 时三者均为 0。
估算以「图中呈现的一份/一盘/可见主体」为基准；若无法合理估算则置 is_food=0 并设置 non_food_code，food_name 与 ai_evaluation 为空字符串，营养字段为 0。`;

/**
 * 分析图片中食物的蛋白质、碳水、摄入热量；强制 JSON 数字字段；非食物走 is_food=0。
...
export async function analyzeFoodNutritionFromImage(
  ...
  let userText =
    '请分析这张图片中的食物（若有）：估算蛋白质、碳水化合物、摄入热量（kcal）；输出具体的 food_name；并撰写充实、分维度完整的 ai_evaluation（勿敷衍短评），严格按系统要求的 JSON 字段与类型。';
